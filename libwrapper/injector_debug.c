...
turn_red(void)
{
	fdprintf(STDOUT_FILENO, "%c[1;31m", 0x1b);
}

static void
turn_blue(void)
{
	fdprintf(STDOUT_FILENO, "%c[1;34m", 0x1b);
}
...
turn_normal(void)
{
	fdprintf(STDOUT_FILENO, "%c[m", 0x1b);
}
       	       ...
       	       const char * fmt, va_list ap)
{
	if (debug_levels[comp] <= level) {
		...
		if (prefix) {
			fdprintf(STDOUT_FILENO, "[%s %s@%s:%d]\t", get_comp_name(comp),
					get_level_name(level), func_name, line_no);
		}

		vfdprintf(STDOUT_FILENO, fmt, ap);
...
vmessage_out(int prefix, enum debug_level l, enum debug_component c, char * fmt, va_list ap)
{
	...
	if (prefix)
		fdprintf(STDOUT_FILENO, "%s: ", get_level_name(l));
	vfdprintf(STDOUT_FILENO, fmt, ap);
